...
const Slide10Calculator = () => {
  ...
  return (
    <SlideLayout>
      <div className="relative w-full h-full flex flex-col items-center justify-center bg-white overflow-hidden px-14 pt-5 pb-4">
        {/* Grid background */}
        ...
        <div className="relative z-10 flex flex-col items-center w-full">
          ...
          <div className="grid grid-cols-[380px_1fr] gap-6 w-full max-w-[1200px] items-center">
            ...
            <div className="space-y-3">
              {/* Location */}
              <div className="rounded-2xl border border-border/50 bg-gradient-to-br from-white to-secondary/30 p-4 shadow-sm">
              ...
              {/* Asset Type */}
              <div className="rounded-2xl border border-border/50 bg-gradient-to-br from-white to-secondary/30 p-4 shadow-sm">
                <label className="text-[11px] font-bold text-foreground uppercase tracking-wider block mb-2.5">Energy Source</label>
              ...
              <button
                ...
                disabled={loading}
                className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-primary to-emerald-600 text-white font-bold text-[14px] flex items-center justify-center gap-2 hover:shadow-xl hover:shadow-primary/20 transition-all disabled:opacity-60 active:scale-[0.98]"
              >
                {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                {loading ? "Analyzing with AI..." : "⚡ Get AI Estimate"}
              </button>
              {error && <p className="text-[11px] text-amber-600 text-center font-medium mt-1">{error}</p>}
            </div>
            ...
            <div className="flex flex-col justify-center gap-3">
              ...
              <div className="rounded-2xl bg-gradient-to-r from-primary via-emerald-600 to-teal-500 p-6 flex items-center justify-between shadow-lg relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <div className="text-[48px] font-black text-white leading-none mt-1">{fmt(totalValue)}</div>
                  <div className="text-[14px] text-white/70 mt-1.5">Over {duration[0]} years • {city}</div>
                </div>
                <div className="relative z-10 flex flex-col items-center gap-1">
                  <TrendingUp className="w-12 h-12 text-white/30" />
                  <AssetIcon className="w-7 h-7 text-white/50" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-4 gap-3">
                {resultCards.map((r) => (
                  <div key={r.label} className="rounded-2xl border border-border/40 bg-white p-4 shadow-sm hover:shadow-md transition-shadow group">
                    ...
                    <div className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide mb-1">{r.label}</div>
                    <div className="text-[24px] font-black text-foreground leading-tight group-hover:text-primary transition-colors">{r.value}</div>
              ...
              {/* AI Insight */}
              {aiData?.notes ? (
                <div className="rounded-2xl bg-gradient-to-r from-primary/5 to-emerald-500/5 border border-primary/15 p-4 flex items-start gap-3">
                  <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-primary to-emerald-600 flex items-center justify-center shrink-0 shadow-sm">
                    <Sparkles className="w-4 h-4 text-white" />
                  </div>
                  <div>
                    <div className="text-[11px] font-bold text-primary uppercase tracking-wider mb-0.5">AI Location Insight</div>
                    <p className="text-[13px] text-foreground leading-relaxed">{aiData.notes}</p>
                  </div>
                </div>
              ) : (
                <div className="rounded-2xl bg-muted/30 border border-border/30 p-3.5 text-center">
        ...
        {/* Bottom tagline — Slide 3 style */}
        <div className="relative z-10 mt-4 animate-fade-in" style={{ animationDelay: "0.3s" }}>
          <div className="flex items-center gap-2 px-6 py-2.5 rounded-full border-2 border-primary/20 bg-primary/5 shadow-sm">
            ...
            <span className="text-[15px] font-semibold text-primary">
              {aiData ? `✓ AI estimates for ${city}` : "Powered by EcoGridia AI • Location-aware energy modeling"}
            </span>
          </div>
        </div>
      </div>
