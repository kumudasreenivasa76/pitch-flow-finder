...
const revenueData = [
  ...
  { year: "FY31", revenue: 148, grossProfit: 122.7, netProfit: 74.0 },
  { year: "FY32", revenue: 235.5, grossProfit: 198.5, netProfit: 122.8 },
  { year: "FY33", revenue: 352, grossProfit: 301.2, netProfit: 190.5 },
  { year: "FY34", revenue: 502, grossProfit: 436.7, netProfit: 281.1 },
  { year: "FY35", revenue: 685, grossProfit: 603.0, netProfit: 393.3 },
];
...
interface YearDetail { year: string; total: string; dc: string; revenue: string; grossProfit: string; netProfit: string; stakeholders: { name: string; count: string; role: string }[]; opex?: string; margin: string; }

const yearDetails: YearDetail[] = [
  { year: "FY26", total: "32", dc: "12", revenue: "$1.8M", grossProfit: "$1.3M", netProfit: "$0.35M", margin: "74%", opex: "$960K",
  ...
  { year: "FY29", total: "465", dc: "215", revenue: "$42.5M", grossProfit: "$34M", netProfit: "$24.8M", margin: "80%", opex: "$9.2M",
  ...
  { year: "FY30", total: "870", dc: "420", revenue: "$85M", grossProfit: "$70M", netProfit: "$41M", margin: "82%", opex: "$14.5M",
  ...
  { year: "FY31", total: "1,280", dc: "620", revenue: "$148M", grossProfit: "$123M", netProfit: "$74M", margin: "83%",
  ...
  { year: "FY32", total: "1,780", dc: "860", revenue: "$236M", grossProfit: "$199M", netProfit: "$123M", margin: "84%",
  ...
  { year: "FY33", total: "2,350", dc: "1,150", revenue: "$352M", grossProfit: "$301M", netProfit: "$191M", margin: "85.5%",
  ...
  { year: "FY34", total: "3,020", dc: "1,480", revenue: "$502M", grossProfit: "$437M", netProfit: "$281M", margin: "87%",
  ...
  { year: "FY35", total: "3,850", dc: "1,920", revenue: "$685M", grossProfit: "$603M", netProfit: "$393M", margin: "88%",
      ...
      <div className="relative w-full h-full flex flex-col items-center justify-center bg-white overflow-hidden px-14 pt-8 pb-5">
        ...
        <div className="relative z-10 w-full">
          <div className="text-center mb-3 animate-fade-in">
            ...
            <h2 className="text-[40px] font-extrabold text-foreground leading-tight" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>EcoGridia — <span className="text-primary">Operating Dashboard</span></h2>
          ...
          <div className="flex justify-center gap-2 mb-3 animate-fade-in" style={{ animationDelay: "0.02s" }}>
            {[{ l: "FY35 Revenue", v: "$685M" },{ l: "FY35 Customers", v: "3,850" },{ l: "FY35 Margin", v: "88%" },{ l: "10Y CAGR", v: "~82%" },{ l: "Net Margin FY35", v: "57%" }].map((k) => (
                ...
                <div className="h-[155px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <ComposedChart data={revenueData}>
                      <defs><linearGradient id="revenueGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.25} /><stop offset="95%" stopColor="#10b981" stopOpacity={0.02} /></linearGradient></defs>
                ...
                <div className="h-[110px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={marginData}>
                      <defs><linearGradient id="marginGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#f59e0b" stopOpacity={0.3} /><stop offset="95%" stopColor="#f59e0b" stopOpacity={0.02} /></linearGradient></defs>
                      <CartesianGrid strokeDasharray="3 3" stroke={gridStroke} />
                      <XAxis dataKey="year" tick={chartTick} tickLine={false} axisLine={false} />
            ...
            <div className="col-span-7 flex flex-col gap-2">
              <div className="bg-card border border-border rounded-lg p-2 animate-fade-in" style={{ animationDelay: "0.15s" }}>
                ...
                <table className="w-full text-[10px]">
                  <thead><tr className="border-b border-border"><th className="text-left py-1 text-muted-foreground font-semibold w-[60px]">Metric</th>
                    {yearDetails.map((r) => (<th key={r.year} className={`text-center py-1 font-semibold cursor-pointer transition-colors ${selectedYear === r.year ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-primary"}`} onClick={() => setSelectedYear(selectedYear === r.year ? null : r.year)}>{r.year}</th>))}
                  </tr></thead>
                  <tbody>
                    {[{ label: "Customers", key: "total" as const, bold: true },{ label: "DC", key: "dc" as const, bold: false },{ label: "Revenue", key: "revenue" as const, bold: true },{ label: "Gross", key: "grossProfit" as const, bold: false },{ label: "Net", key: "netProfit" as const, bold: false }].map((row) => (
                      <tr key={row.label} className="border-b border-border/50 last:border-0">
                        <td className="py-1 font-medium text-foreground">{row.label}</td>
                        {yearDetails.map((r) => (<td key={r.year} className={`text-center py-1 ${row.bold ? "font-bold" : "font-semibold"} ${selectedYear === r.year ? "text-primary bg-primary/5" : row.bold ? "text-primary" : "text-foreground"}`}>{r[row.key]}</td>))}
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              {detail ? (
                <div className="bg-primary/5 border-2 border-primary/30 rounded-lg p-2 animate-fade-in">
                  <div className="flex items-center justify-between mb-1"><MiniLabel>🔍 {detail.year} Breakdown</MiniLabel><button onClick={() => setSelectedYear(null)} className="text-[9px] text-muted-foreground hover:text-foreground transition-colors underline">Close</button></div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <div className="text-[9px] font-bold text-muted-foreground uppercase mb-0.5">Financials</div>
                      <div className="grid grid-cols-2 gap-1">
                        {[{ l: "Revenue", v: detail.revenue },{ l: "Gross Profit", v: detail.grossProfit },{ l: "Net Profit", v: detail.netProfit },{ l: "Margin", v: detail.margin },{ l: "Customers", v: detail.total },...(detail.opex ? [{ l: "OPEX", v: detail.opex }] : [])].map((m) => (
                          <div key={m.l} className="bg-card border border-border rounded px-1.5 py-0.5 text-center"><div className="text-[11px] font-bold text-primary">{m.v}</div><div className="text-[8px] text-muted-foreground">{m.l}</div></div>
                        ))}
                      </div>
                    </div>
                    <div>
                      <div className="text-[9px] font-bold text-muted-foreground uppercase mb-1">Stakeholder Acquisition</div>
                      <table className="w-full text-[10px]"><thead><tr className="border-b border-border"><th className="text-left py-0.5 text-muted-foreground font-semibold">Segment</th><th className="text-center py-0.5 text-muted-foreground font-semibold">#</th><th className="text-right py-0.5 text-muted-foreground font-semibold">Role</th></tr></thead>
                        <tbody>{detail.stakeholders.map((s) => (<tr key={s.name} className="border-b border-border/30"><td className="py-0.5 font-medium text-foreground">{s.name}</td><td className="text-center py-0.5 font-bold text-primary">{s.count}</td><td className="text-right py-0.5 text-[9px] text-muted-foreground">{s.role}</td></tr>))}</tbody>
                      </table>
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-amber-50/50 border-2 border-amber-400/30 rounded-lg p-2 animate-fade-in" style={{ animationDelay: "0.2s" }}>
                    ...
                    <div className="space-y-1">
                      {[{ l: "Headcount", v: "10 → 240 FTE", sub: "FY26 → FY35" },{ l: "CAC Trend", v: "Declining", sub: "Scale efficiencies" },{ l: "NRR", v: "> 120%", sub: "Upsell + expansion" },{ l: "Expansion Rate", v: "35% / yr", sub: "Multi-site + credits" },{ l: "Churn", v: "12% annual", sub: "Contractual lock-in" }].map((a) => (
                        <div key={a.l} className="flex items-center justify-between"><span className="text-[10px] text-muted-foreground">{a.l}</span><div className="text-right"><span className="text-[11px] font-bold text-foreground">{a.v}</span><span className="text-[8px] text-muted-foreground ml-1">{a.sub}</span></div></div>
                      ))}
                    </div>
                  </div>
                  <div className="bg-primary/5 border-2 border-primary/20 rounded-lg p-2 animate-fade-in" style={{ animationDelay: "0.25s" }}>
                    <MiniLabel>🎯 Near-Term Milestones</MiniLabel>
                    <div className="space-y-1.5">
                      {[{ y: "FY26", t: "Launch + first 32 customers" },{ y: "FY27", t: "95 customers, $6.5M revenue" },{ y: "FY28", t: "225 customers, Series A" },{ y: "FY30", t: "870 customers, $85M rev" }].map((m) => (
                        <div key={m.y} className="flex gap-2"><span className="text-[10px] font-bold text-primary shrink-0 w-8">{m.y}</span><span className="text-[10px] text-foreground">{m.t}</span></div>
                      ))}
                    </div>
                  </div>
                </div>
